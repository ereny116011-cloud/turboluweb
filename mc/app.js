...
const translations = {
  tr: {
    ...
    requests: 'Talepler', addAnnouncement: 'Duyuru Ekle',
    manageAnnouncements: 'Duyuru Yönet', addCampaign: 'Kampanya Ekle',
    manageCampaigns: 'Kampanya Yönet', addNews: 'Haber Ekle',
    manageNews: 'Haber Yönet', addItem: 'Ürün Ekle',
  },
  en: {
    shop: 'Shop', campaigns: 'Campaigns', announcements: 'Announcements',
    news: 'News', inventory: 'My Requests', profile: 'Profile',
    logout: 'Logout', register: 'Register', login: 'Login',
    requests: 'Requests', addAnnouncement: 'Add Announcement',
    manageAnnouncements: 'Manage Announcements', addCampaign: 'Add Campaign',
    manageCampaigns: 'Manage Campaigns', addNews: 'Add News',
...
function renderUI() {
  ...
  if (currentUser) {
    ...
    userArea.innerHTML = `
      ...
      <button class="btn" onclick="openAuthModal('login')">Giriş Yap</button>
    `;
  }
...
async function renderRequests() {
  ...
  const content = document.getElementById('content');
  content.innerHTML = `<div class="glass-card"><h2>📋 Talepler</h2>${requests.length===0?'<p>Talep yok.</p>':requests.map(r=>`<div style="padding:10px; background:rgba(0,0,0,0.3); border-radius:8px; margin:5px 0;"><div style="display:flex; justify-content:space-between;"><div><b>${r.user}</b> → ${r.item} (${r.price} puan)<br><small>${new Date(r.date).toLocaleString()}</small></div><div>${r.status==='pending'?`<button onclick="completeRequest('${r.id}')">✅</button><button onclick="rejectRequest('${r.id}')" style="background:rgba(239,68,68,0.8);">❌</button>`:`<span>${r.status}</span>`}</div></div></div>`).join('')}</div>`;
}
...
async function renderManageNews() {
  ...
  const content = document.getElementById('content');
  const news = await fetch(`${API}/api/news`).then(r => r.json());
  content.innerHTML = `<div class="glass-card"><h2>📋 Haber Yönet</h2>${news.length === 0 ? '<p>Henüz haber yok.</p>' : news.map(n => `<div style="padding:10px; background:rgba(0,0,0,0.3); border-radius:8px; margin:5px 0; display:flex; justify-content:space-between; align-items:center;"><div><b>${n.title}</b><br><small>${new Date(n.date).toLocaleString()}</small></div><button onclick="deleteNews('${n.id}')" style="background:rgba(239,68,68,0.8);">🗑️ Sil</button></div>`).join('')}</div>`;
}
...
async function renderManageCampaigns() {
  ...
  const campaigns = await fetch(`${API}/api/campaigns`).then(r => r.json());
  const now = new Date();
  document.getElementById('content').innerHTML = `<div class="glass-card"><h2>📊 Kampanya Yönet</h2>${campaigns.map(c=>{const expired=c.endDate&&new Date(c.endDate)<now;return`<div class="${expired?'campaign-expired':'campaign-active'}" style="padding:10px; background:rgba(0,0,0,0.3); border-radius:8px; margin:5px 0; display:flex; justify-content:space-between;"><div><b>${c.title}</b><br><small>${c.description} | 🎁 ${c.reward}</small><br><small>📅 ${c.endDate?new Date(c.endDate).toLocaleString():'Süresiz'} ${expired?'⚠️ Süresi Doldu':''}</small></div><button onclick="deleteCampaign('${c.id}')" style="background:rgba(239,68,68,0.8);">🗑️ Sil</button></div>`}).join('')}</div>`;
}
...
function openAuthModal(mode) {
  ...
  body.innerHTML = `
    <h3>${isRegister ? 'Kaydol' : 'Giriş Yap'}</h3>
    ...
    <button class="btn-green" id="authSubmit">${isRegister ? 'Kaydol' : 'Giriş'}</button>
